...
class S18
  ...
  def creation(options)
    ...
    StagingRecords[STAGING_TABLE, options].each do |stage|
      ...
      sample[:teaser] = "Here #{committees_num == 1 ? "is" : "are"} the #{Numeric.digit_to_string(committees_num)} political #{plural('committee', committees_num)} that received #{committees_num >=50 ? "most money from contributions" : "money from contributions"} during #{month_name(month)}."

      output = "Here #{committees_num == 1 ? "is" : "are"} the #{Numeric.digit_to_string(committees_num)} political "\
